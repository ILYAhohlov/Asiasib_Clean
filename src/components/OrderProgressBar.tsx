...
export function OrderProgressBar({ currentStep }: OrderProgressBarProps) {
  ...
  const validStep = Math.max(1, Math.min(currentStep || 1, steps.length));
                  ...
                  <div className="flex items-center text-sm">
                    ...
                    <span
                      className={`ml-3 font-medium transition-colors duration-300 ${
                        validStep > step.id ? "text-green-600" :
                        validStep === step.id ? "text-primary" : "text-muted-foreground"
                      }`}
                    >
                      {step.name}
                    </span>
                  ...
                  {stepIdx < steps.length - 1 && (
                    <div
                      className={`flex-1 h-1 ml-4 rounded-full transition-all duration-500 ${
                        validStep > step.id ? "bg-gradient-to-r from-green-500 to-emerald-600 shadow-sm" : "bg-gray-200"
                      }`}
                      aria-hidden="true"
                    />
                  )}
